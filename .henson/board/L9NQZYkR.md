...
- `RunManager` writes each run (output included) to `.henson/runs/<runId>.json`
  on start and on completion; `hydrate()` reloads them on `henson serve` startup.
...
- `.henson/runs/` is gitignored (large, machine-specific logs); README updated.
