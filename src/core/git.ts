...
export interface Commit {
  ...
  date: string; // ISO
  subject: string;
  /** Companion name parsed from a `Henson-Companion:` trailer, if present. */
  companion?: string;
}
...
/**
 * Recent git commits in a project, with the `Henson-Companion:` trailer parsed
  ...
  try {
    ...
    return stdout
      ...
      .map((chunk) => {
        ...
        const trailer = body.match(/^Henson-Companion:\s*(.+?)\s*$/im);
