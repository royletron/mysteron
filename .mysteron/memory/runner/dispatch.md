...
`runsOn` enforcement gotcha (ticket `HrzvZXYS`): the host-pin checks must consult the companion a run is *attributed* to, which for an **unassigned** ticket is the soloist (`defaultCompanion`), not nobody. The planner's `companionFor` therefore falls back to `defaultCompanion(config)` — same fallback as `runningCompanionId` and `manager.startOnWorker`. Without the fallback, `getCompanion(config, undefined)` returned `undefined`, `companionAllowsGuest(undefined, ...)` was vacuously true, and unassigned work fanned out to *any* guest, ignoring a soloist pinned to `local`. The manual-run path in `api.ts` already had the fallback; the planner didn't.
